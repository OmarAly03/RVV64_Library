...
void leaky_relu_e32m1(const float* src, float* dest, size_t n, float alpha) {
    ...
    while (n >= vl * 8) {
        ...
        VECTOR_STORE<float, M1>(dest + vl*0, VECTOR_MUL_MASKED<float, M1>(VECTOR_LT_SCALAR<float, M1>(v0, 0.0f, vl), v0, alpha, vl), vl);
        VECTOR_STORE<float, M1>(dest + vl*1, VECTOR_MUL_MASKED<float, M1>(VECTOR_LT_SCALAR<float, M1>(v1, 0.0f, vl), v1, alpha, vl), vl);
        VECTOR_STORE<float, M1>(dest + vl*2, VECTOR_MUL_MASKED<float, M1>(VECTOR_LT_SCALAR<float, M1>(v2, 0.0f, vl), v2, alpha, vl), vl);
        VECTOR_STORE<float, M1>(dest + vl*3, VECTOR_MUL_MASKED<float, M1>(VECTOR_LT_SCALAR<float, M1>(v3, 0.0f, vl), v3, alpha, vl), vl);
        VECTOR_STORE<float, M1>(dest + vl*4, VECTOR_MUL_MASKED<float, M1>(VECTOR_LT_SCALAR<float, M1>(v4, 0.0f, vl), v4, alpha, vl), vl);
        VECTOR_STORE<float, M1>(dest + vl*5, VECTOR_MUL_MASKED<float, M1>(VECTOR_LT_SCALAR<float, M1>(v5, 0.0f, vl), v5, alpha, vl), vl);
        VECTOR_STORE<float, M1>(dest + vl*6, VECTOR_MUL_MASKED<float, M1>(VECTOR_LT_SCALAR<float, M1>(v6, 0.0f, vl), v6, alpha, vl), vl);
        VECTOR_STORE<float, M1>(dest + vl*7, VECTOR_MUL_MASKED<float, M1>(VECTOR_LT_SCALAR<float, M1>(v7, 0.0f, vl), v7, alpha, vl), vl);
    ...
    while (n > 0) {
        ...
        VECTOR_STORE<float, M1>(dest, VECTOR_MUL_MASKED<float, M1>(VECTOR_LT_SCALAR<float, M1>(v, 0.0f, vl), v, alpha, vl), vl);
        src += vl; dest += vl; n -= vl;
    }
}
...
void leaky_relu_e32m2(const float* src, float* dest, size_t n, float alpha) {
	...
	while (n >= vl * 4) {
		...
		VECTOR_STORE<float, M2>(dest + vl*0, VECTOR_MUL_MASKED<float, M2>(VECTOR_LT_SCALAR<float, M2>(v0, 0.0f, vl), v0, alpha, vl), vl);
		VECTOR_STORE<float, M2>(dest + vl*1, VECTOR_MUL_MASKED<float, M2>(VECTOR_LT_SCALAR<float, M2>(v1, 0.0f, vl), v1, alpha, vl), vl);
		VECTOR_STORE<float, M2>(dest + vl*2, VECTOR_MUL_MASKED<float, M2>(VECTOR_LT_SCALAR<float, M2>(v2, 0.0f, vl), v2, alpha, vl), vl);
		VECTOR_STORE<float, M2>(dest + vl*3, VECTOR_MUL_MASKED<float, M2>(VECTOR_LT_SCALAR<float, M2>(v3, 0.0f, vl), v3, alpha, vl), vl);
	...
	while (n > 0) {
		...
		VECTOR_STORE<float, M2>(dest, VECTOR_MUL_MASKED<float, M2>(VECTOR_LT_SCALAR<float, M2>(v, 0.0f, vl), v, alpha, vl), vl);
		src += vl; dest += vl; n -= vl;
	}
}
...
void leaky_relu_e32m4(const float* src, float* dest, size_t n, float alpha) {
	...
	while (n >= vl * 2) {
		...
		VECTOR_STORE<float, M4>(dest + vl*0, VECTOR_MUL_MASKED<float, M4>(VECTOR_LT_SCALAR<float, M4>(v0, 0.0f, vl), v0, alpha, vl), vl);
		VECTOR_STORE<float, M4>(dest + vl*1, VECTOR_MUL_MASKED<float, M4>(VECTOR_LT_SCALAR<float, M4>(v1, 0.0f, vl), v1, alpha, vl), vl);
	...
	while (n > 0) {
		...
		VECTOR_STORE<float, M4>(dest, VECTOR_MUL_MASKED<float, M4>(VECTOR_LT_SCALAR<float, M4>(v, 0.0f, vl), v, alpha, vl), vl);
		src += vl; dest += vl; n -= vl;
	}
}
...
void leaky_relu_e32m8(const float* src, float* dest, size_t n, float alpha) {
	...
	while (n >= vl * 2) {
		...
		VECTOR_STORE<float, M8>(dest + vl*0, VECTOR_MUL_MASKED<float, M8>(VECTOR_LT_SCALAR<float, M8>(v0, 0.0f, vl), v0, alpha, vl), vl);
		VECTOR_STORE<float, M8>(dest + vl*1, VECTOR_MUL_MASKED<float, M8>(VECTOR_LT_SCALAR<float, M8>(v1, 0.0f, vl), v1, alpha, vl), vl);
	...
	while (n > 0) {
		...
		VECTOR_STORE<float, M8>(dest, VECTOR_MUL_MASKED<float, M8>(VECTOR_LT_SCALAR<float, M8>(v, 0.0f, vl), v, alpha, vl), vl);
		src += vl; dest += vl; n -= vl;
	}
}
